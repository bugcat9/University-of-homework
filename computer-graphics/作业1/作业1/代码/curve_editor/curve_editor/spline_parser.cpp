...
Surface* SplineParser::ParseSurfaceOfRevolution() {
  // a surface of revolution is just a curve spun around  旋转表面只是围绕旋转的曲线
  // the y axis (x = 0, z = 0)
  char token[MAX_PARSER_TOKEN_LENGTH];
  getToken(token);
  Curve *c = NULL;
...
Surface* SplineParser::ParseBezierPatch() {
  // a bezier patch is just a list of 16 vertices bezier patch是一个包含16个顶点的列表
...
void SplineParser::Pick(float x, float y, float epsilon, 
  ...
  // iterate through all the splines  遍历所有样条
  for (int i = 0; i < num_splines; i++) {
    ...
    for (int j = 0; j < s->getNumVertices(); j++) {
      ...
      // keep the closest point (as long as it's within epsilon) 保持最近的点（只要它在epsilon内）
      if (d < distance) {
	selected_curve = s;
